...
class MuscleScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text('Muscle Category: ${cat.name.camelToTitle()}')),
      body: SingleChildScrollView(
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            ...muscles
                .where((m) => m.categories.contains(cat))
                .map(
                  (m) => Padding(
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        table(
          rows: [
            if (muscle.nick.isNotEmpty)
              DataRow(
                cells: [const DataCell(Text('nicknames')), DataCell(Text(muscle.nick.join(', ')))],
              ),
        ...
        if (muscle is MultiHeadMuscle)
          ...(muscle as MultiHeadMuscle).headsMap.values
        ...
        SingleChildScrollView(
          ...
          child: DataTable(
            ...
            columns: [
              ...
              ...muscle.heads.map(
                (h) => DataColumn(label: Text(h.name.camelToTitle().replaceAll(", ", "\n"))),
              ),
            ],
            rows: muscle
                .getArticulations()
        ...
        cells: [
          ...
          DataCell(
            Padding(
              ...
              child: InsufficiencyWidget(insufficiency: head.activeInsufficiency!),
            ),
          ),
        ],
      ),
      ...
      DataRow(
        cells: [
          ...
          DataCell(
            Padding(
              padding: const EdgeInsets.only(top: 8, bottom: 8),
              child: InsufficiencyWidget(insufficiency: head.passiveInsufficiency!),
            ),
          ),
        ],
      ),
